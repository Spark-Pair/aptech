...
class AttendanceReport
{
    ...
    {
        $range=$this->range($month); $branch=$filters['branch']??null;
        $scope=fn($q)=>$q->whereBetween('date',$range)->when($branch,fn($q,$id)=>$q->where('branch_id',$id));
        return Employee::query()->with('shift')->with(['attendance'=>fn($q)=>$scope($q)->select('id','branch_id','empid','date','status','check_in','check_out')])
            ...
            ->when($branch,fn($q,$id)=>$q->whereHas('attendance',fn($a)=>$a->whereBetween('date',$range)->where('branch_id',$id)))
            ->withCount([
                'attendance as working_days'=>fn($q)=>$scope($q)->where('status','!=','Off Day'),
                'attendance as present_days'=>fn($q)=>$scope($q)->where('status','Present'),
                'attendance as absent_days'=>fn($q)=>$scope($q)->where('status','Absent'),
                'attendance as off_days'=>fn($q)=>$scope($q)->where('status','Off Day'),
                'attendance as leave_days'=>fn($q)=>$scope($q)->where('status','Leave'),
            ]);
    }
    public function attendance(string $month,array $filters=[]): Builder
    {
        return Attendance::query()->with(['employee.shift','branch'])->whereBetween('date',$this->range($month))
            ->when($filters['branch']??null,fn($q,$id)=>$q->where('branch_id',$id))
