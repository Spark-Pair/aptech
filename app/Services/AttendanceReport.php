...
{
    public function range(string $month): array
    {
    ...
    public function employees(string $month, array $filters = []): Builder
    {
        ...
        return Employee::query()
            ->with(['attendance' => fn ($q) => $q->whereBetween('date', $range)->select('id', 'empid', 'date', 'status', 'check_in', 'check_out')])
            ->when($filters['search'] ?? null, fn ($q, $value) => $q->where(fn ($q) => $q->where('name', 'like', '%'.$value.'%')->orWhere('empid', $value)))
    ...
    {
        return Attendance::query()->whereBetween('date', $this->range($month))
    ...
    public function summary(Builder $query): array
    {
        ...
        return ['Present' => (int) ($counts['Present'] ?? 0), 'Absent' => (int) ($counts['Absent'] ?? 0),
            'Off Day' => (int) ($counts['Off Day'] ?? 0), 'Leave' => (int) ($counts['Leave'] ?? 0)];
    }
}
